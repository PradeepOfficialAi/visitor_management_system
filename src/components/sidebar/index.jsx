...
const SideBar = () => {
  ...
  return (
    <aside className={`bg-secondary-dark transition-width duration-300 ${isCollapsed ? 'w-16' : 'w-56'}`} aria-label="Sidebar">
        ...
        <nav>
          {navItems.map((item) => (
            <Link
              to={item.path}
              key={item.name}
              className={`flex items-center p-2 text-base font-normal text-white rounded-lg hover:bg-accent-blue ${location.pathname === item.path ? 'bg-accent-blue' : ''
                }`}
            >
