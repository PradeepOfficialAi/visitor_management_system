...
const CreateNewPass = ({ open, onClose, visitor }) => {
    ...
    useEffect(() => {
        ...
        setKeyList(db.keys);
        setZoneList(db.zones.map(zone => ({ id: zone.id, name: zone.zone_name })));
    }, [open]);
    ...
    const handleSubmit = () => {
        if (!validate()) return;
        const db = localStorageManager.getDB();
        const newPass = { ...passData, id: Date.now(), created_on: new Date().toISOString(), visitor: visitor };
            ...
            >
                ...
                <div className="flex items-center justify-between p-3">
                    {steps.map((label, index) => (
                        <div
                            key={label}
                            className={`flex-1 ${index <= activeStep ? "bg-green-500" : "bg-gray-200"} h-2 mx-2 rounded-full transition duration-500 ease-in-out`}
                        ></div>
                    ))}
                </div>
