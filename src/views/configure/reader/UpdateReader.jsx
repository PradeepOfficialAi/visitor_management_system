...
const UpdateReader = ({ open, onClose, fetchData, readerData }) => {
    ...
    useEffect(() => {
        ...
        setAdamList(db.adams.map(adam => ({ id: adam.id, ip: adam.ip, name: adam.name })));
        setZoneList(db.zones.map(zone => ({ id: zone.id, zoneName: zone.zone_name })));
    }, []);
    ...
    const handleSubmit = () => {
        if (!validate()) return;
        const db = localStorageManager.getDB();
        const index = db.readers.findIndex(reader => reader.id === formData.id);
        if (index !== -1) {
            db.readers[index] = formData;
